...
contract VolumeTrackerHook is BaseHook, Access, Option {
    ...
    function afterSwap(
        ...
        if (liquidity > threshold) {
            ...
            // This is the decreasing straight line of the piecewise function that is obtained from the formula above
            strikePrice =
    ...
    function updateMin(uint256 newMin) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newMin >= 12 && newMin < max, "VolumeTrackerHook: INVALID_MIN");

        min = newMin;
    }

    function updateMax(uint256 newMax) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newMax > min && newMax <= 32, "VolumeTrackerHook: INVALID_MAX");
    ...
    /**
     *
     * @param tokenId option otken to void
     */
    function _voidOptionByTokenId(uint256 tokenId) internal override {
        ...
        if (tokenId2Option[tokenId].void) return; // do nothing if it is already voided
    ...
    function _voidOptionByExpiryPrice(uint256 expiryPrice_) internal override {
        ...
        if (spotPrice <= expiryPrice_) {
            ...
            // This is optional since the set will be empty and won't consume much gas
            // but it might be useful to remove the mapping entry entirely
